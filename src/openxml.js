...
const XEnumerable = require('./ltxml-extensions');
...
(function (root) {
  ...
  function defineOpenXml(Enumerable, Ltxml, XEnumerable) {
    ...
    const XCData = Ltxml.XCData;
    const XComment = Ltxml.XComment;
    const XContainer = Ltxml.XContainer;
    const XDeclaration = Ltxml.XDeclaration;
    const XDocument = Ltxml.XDocument;
    const XElement = Ltxml.XElement;
    const XEntity = Ltxml.XEntity;
    const XName = Ltxml.XName;
    const XNamespace = Ltxml.XNamespace;
    const XNode = Ltxml.XNode;
    const XObject = Ltxml.XObject;
    const XProcessingInstruction = Ltxml.XProcessingInstruction;
    ...
    async function openFromZip(zip, pkg) {
      ...
      for (const f in zip.files) {
        ...
        if (!openXml.util.endsWith(f, "/")) {
          ...
          if (f !== "[Content_Types].xml")
            f2 = "/" + f;
          const newPart = new openXml.OpenXmlPart(pkg, f2, null, null, zipFile.async('string'));
          promises.push(newPart.data);
          parts.push(newPart);
      ...
      const ctf = pkg.parts["[Content_Types].xml"];
      if (ctf === null) {
      ...
      for (const part in pkg.parts) {
        if(pkg.parts.hasOwnProperty(part)) {
          if (part === "[Content_Types].xml")
            continue;
          const ct = pkg.getContentType(part);
          const thisPart = pkg.parts[part];
    ...
    function openFlatOpcFromXDoc(pkg, doc) {
      ...
      root.elements(FLATOPC.part).forEach(function (p) {
        const uri = p.attribute(FLATOPC.name).value;
        const contentType = p.attribute(FLATOPC.contentType).value;
        let partType = "xml";
        if (!openXml.util.endsWith(contentType, "xml"))
          partType = "base64";
        if (partType === "xml") {
          pkg.parts[uri] = new openXml.OpenXmlPart(pkg, uri, contentType, partType,
        ...
        if (partType === "base64") {
          pkg.parts[uri] = new openXml.OpenXmlPart(pkg, uri, contentType, partType, p.element(FLATOPC.binaryData).value);
          pkg.ctXDoc.getRoot().add(
            new XElement(CT.Override,
              new XAttribute("PartName", uri),
              new XAttribute("ContentType", contentType)));
      ...
      pkg.parts["[Content_Types].xml"] = new openXml.OpenXmlPart(pkg, "[Content_Types].xml", null, "xml", pkg.ctXDoc);
    }

    function openFromFlatOpcInternal(pkg, flatOpc) {
      const xDoc = XDocument.parse(flatOpc);
    ...
    openXml.OpenXmlPackage.prototype.openFromBase64 = function (base64data, cb) {
      return openFromBase64InternalAsync(this, base64data, cb);
    };
    ...
    openXml.OpenXmlPackage.prototype.openFromBlob = function (blob, cb) {
      return openFromBlobInternalAsync(this, blob, cb);
    };

    function saveToZip(that, type) {
      const zip = new JSZip();
      for (const part in that.parts) {
        let ct;
        const thisPart = that.parts[part];
        // test for null because if the part was deleted, it will be set to null, not undefined
        if (that.parts.hasOwnProperty(part) && thisPart !== null) {
          if (part === "[Content_Types].xml") {
            ...
            if (cte === null) {
              const extension = part
                ...
                .toLowerCase();
              const dct = that.ctXDoc.getRoot().elements(CT.Default).firstOrDefault(function (e) {
            ...
            if (thisPart.partType === "base64") {
              const nsp = thisPart.data;
              const sp = [];
              let pos = 0;
              const len = nsp.length;
              while (true) {
                if (pos >= len) {
                  break;
                }
    ...
    openXml.OpenXmlPackage.prototype.saveToFlatOpc = function () {
      const xdec = new XDeclaration("1.0", "UTF-8", "yes");
      const nsa = new XAttribute(XNamespace.xmlns + "pkg",
        "http://schemas.microsoft.com/office/2006/xmlPackage");
      const pkg = new XElement(FLATOPC._package, nsa);
      const flatOpc = new XDocument(xdec,
        new XProcessingInstruction('mso-application', 'progid="Word.Document"'),
        pkg);
      for (const part in this.parts) {
        if (this.parts.hasOwnProperty(part) && part !== "[Content_Types].xml") {
          let ct;
          ...
          if (thisPart !== null) {
            ...
            const nameAtt = new XAttribute(FLATOPC.name, part);
            const ctAtt = new XAttribute(FLATOPC.contentType, ct);
            let compressionAtt = null;
            if (compression) {
            ...
            const partElement = new XElement(FLATOPC.part, nameAtt, ctAtt, compressionAtt, dataElement);
            pkg.add(partElement);
          }
        }
      }
    ...
    openXml.OpenXmlPackage.prototype.saveToFlatOpcAsync = function (cb) {
      ...
      let count = 0;
      const docpkg = this;
      const partCount = partQueue.length;

      const intervalId = setInterval(function () {
        if (count < partCount) {
          for (let i = 0; i < 10; i++) {
            const thisPart = partQueue[count];
            const cte = docpkg.ctXDoc.getRoot().elements(CT.Override).firstOrDefault(function (e) {
              const ctpn = e.attribute("PartName").value;
              const thpn = thisPart.uri;
              return ctpn === thpn;
            });
            let ct = null;
            if (!cte) {
            ...
            const nameAtt = new XAttribute(FLATOPC.name, thisPart.uri);
            const ctAtt = new XAttribute(FLATOPC.contentType, ct);
            let compressionAtt = null;
            if (compression) {
              compressionAtt = new XAttribute(FLATOPC.compression, compression);
    ...
    openXml.OpenXmlPackage.prototype.addPart = function (uri, contentType, partType, data) {
      const ctEl = this.ctXDoc.getRoot().elements(CT.Override)
        .firstOrDefault(function (or) {
          const orn = or.attribute("PartName").value;
          return uri === orn;
        });
      ...
      const newPart = new openXml.OpenXmlPart(this, uri, contentType, partType, data);
    ...
    function getRelationshipsFromRelsXml(pkg, part, relationshipPart) {
      const rxDoc = relationshipPart.getXDocument();
      return rxDoc.getRoot().elements(PKGREL.Relationship)
        .select(function (r) {
          let targetMode = null;
          const tm = r.attribute("TargetMode");
          if (tm) {
            targetMode = tm.value;
          }
    ...
    openXml.OpenXmlPackage.prototype.getRelationships = function () {
      ...
      return getRelationshipsFromRelsXml(this, null, rootRelationshipsPart);
    };

    openXml.OpenXmlPackage.prototype.getParts = function () {
      const parts = [];
      for (const part in this.parts) {
        if (this.parts.hasOwnProperty(part) && this.parts[part].contentType !== openXml.contentTypes.relationships &&
    ...
    openXml.OpenXmlPackage.prototype.getPartsByRelationshipType = function (relationshipType) {
      ...
      for (let i = 0; i < rels.length; ++i) {
        const part = this.getPartByUri(rels[i].targetFullName);
        parts.push(part);
      }
      return parts;
    };
    ...
    openXml.OpenXmlPackage.prototype.getRelationshipsByContentType = function (contentType) {
      ...
      if (rootRelationshipsPart) {
        ...
        for (let i = 0; i < allRels.length; ++i) {
          ...
          const ct = this.getContentType(allRels[i].targetFullName);
          if (ct !== contentType) {
            continue;
          }
          rels.push(allRels[i]);
    ...
    openXml.OpenXmlPackage.prototype.getRelationshipById = function (rId) {
      const rel = Enumerable.from(this.getRelationships())
    ...
    openXml.OpenXmlPackage.prototype.getPartById = function (rId) {
      ...
      return this.getPartByUri(rel.targetFullName);
    };
    ...
    openXml.OpenXmlPackage.prototype.addRelationship = function (relationshipId, relationshipType, target, targetMode) {
      ...
      if (!rootRelationshipPart) {
        rootRelationshipPart = this.addPart("/_rels/.rels", this.contentTypes.relationships, "xml",
          new XDocument(
    ...
    openXml.OpenXmlPackage.prototype.deleteRelationship = function (relationshipId) {
      const rootRelationshipsPart = this.getPartByUri("/_rels/.rels");
      const rxDoc = rootRelationshipsPart.getXDocument();
      const rxe = rxDoc.getRoot().elements(PKGREL.Relationship)
        .firstOrDefault(function (r) {
    ...
    openXml.OpenXmlPackage.prototype.contentParts = function () {
      ...
      const pa = [main];
      const headers = main.headerParts();
      for (let i = 0; i < headers.length; i++) {
        pa.push(headers[i]);
      }
      const footers = main.footerParts();
    ...
    openXml.OpenXmlPart.prototype.getXDocument = function () {
      if (this.partType === 'xml' && typeof this.data === 'string') {
        ...
        const utf = openXml.util.decode_utf8(data);
    ...
    function getRelsPartUriOfPart(part) {
      ...
      const partFileName = uri.substring(lastSlash + 1);
      const relsFileName = uri.substring(0, lastSlash) + "/_rels/" + partFileName + ".rels";
      return relsFileName;
    }

    function getRelsPartOfPart(part) {
      const relsFileName = getRelsPartUriOfPart(part);
      const relsPart = part.pkg.getPartByUri(relsFileName);
      return relsPart;
    }

    // returns all relationships
    openXml.OpenXmlPart.prototype.getRelationships = function () {
      const relsPart = getRelsPartOfPart(this);
      if (relsPart) {
        return getRelationshipsFromRelsXml(null, this, relsPart);
      }
      return [];
    };
    ...
    openXml.OpenXmlPart.prototype.getParts = function () {
      ...
      for (let i = 0; i < rels.length; ++i) {
        if (rels[i].targetMode === "Internal") {
          const part = this.pkg.getPartByUri(rels[i].targetFullName);
          parts.push(part);
        }
      }
      return parts;
    ...
    openXml.OpenXmlPart.prototype.getRelationshipsByContentType = function (contentType) {
      ...
      for (let i = 0; i < allRels.length; ++i) {
        if (allRels[i].targetMode === "Internal") {
          const ct = this.pkg.getContentType(allRels[i].targetFullName);
          if (ct === contentType) {
            rels.push(allRels[i]);
          }
        }
    ...
    openXml.OpenXmlPart.prototype.getPartById = function (relationshipId) {
      ...
      if (rel) {
        return this.pkg.getPartByUri(rel.targetFullName);
      }
      return null;
    };
    ...
    openXml.OpenXmlPart.prototype.addRelationship = function (relationshipId, relationshipType, target, targetMode) {
      let relsPart = getRelsPartOfPart(this);
      if (!relsPart) {
        let relsPartUri = getRelsPartUriOfPart(this);
    ...
    openXml.OpenXmlPart.prototype.deleteRelationship = function (relationshipId) {
      ...
      if (relsPart) {
        ...
        const theRel = relsPartXDoc.getRoot().elements(PKGREL.Relationship).firstOrDefault(function (r) {
    ...
    openXml.OpenXmlRelationship = function (pkg, part, relationshipId, relationshipType, target, targetMode) {
      ...
      if (this.fromPart) {
        const slashIndex = this.fromPart.uri.lastIndexOf('/');
        if (slashIndex === -1) {
          workingCurrentPath = "/";
        }
        else {
      ...
      while (openXml.util.startsWith(workingTarget, '../')) {
        ...
        const indexOfLastSlash = workingCurrentPath.lastIndexOf('/');
    ...
    const bucketTimer = openXml.util.bucketTimer;
    ...
    bucketTimer.init = function () {
      ...
      for (prop in bucketTimer) {
        if (bucketTimer.hasOwnProperty(prop) && typeof bucketTimer[prop] !== 'function' &&
          prop !== 'currentTime' &&
          prop !== 'currentBucket' &&
          prop !== 'beginningTime') {
    ...
    openXml.util.lPad = function (str, padLength, padChar) {
      ...
      const p = Math.max((padLength + 1) - str.length, 0);
      return new Array(p).join(padChar) + str;
    };
    ...
    openXml.util.rPad = function (str, padLength, padChar) {
      ...
      // is padLength in length
      const p = Math.max((padLength + 1) - str.length, 0);  //ignore jslint
      return str + new Array(p).join(padChar);
    };

    bucketTimer.dump = function () {
      let prop, s, a, a2, a3, a4, diffFromBeginning, i;
    ...
    function getChar(input, i) {
      if (input.charAt(i) === '%') {
        i++;
        let h1 = input.charAt(i++);
        h1 += input.charAt(i++);
        const chr = parseInt(h1, 16);
        return {
          chr: chr,
          i: i
        }
    ...
    function convertToEntities(text) {
      let o, len, i, c, cs, em, grouped, retValue, str;
      ...
      for (i = 0; i < len; i += 1) {
        ...
        if (c >= 128) {
          ...
          const zz = c.toString(16);//ignore jslint
    ...
    function transformToEntities(node) {
      if (node.nodeType === 'Element') {
        return new XElement(node.name,
          node.attributes(),
          node.nodes().select(function (n) {
            return transformToEntities(n);
          }));
      }
  ...
  if (typeof define === typeof function () {
    ...
    module.exports = defineOpenXml(Enumerable, Ltxml, XEnumerable);
  }
  else {
    // Enumerable, Ltxml, and XEnumerable must be defined before including ltxml.js.
    root.openXml = defineOpenXml(Enumerable, Ltxml, XEnumerable);
  }
